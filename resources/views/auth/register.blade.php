...
<div class="min-h-screen flex items-center justify-center bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
    <div class="max-w-md w-full space-y-8">
        ...
        <form class="mt-8 space-y-6" action="{{ route('register') }}" method="POST">
            ...
            <div class="space-y-4">
                <div>
                    <label for="name" class="block text-sm font-medium text-gray-700 mb-1">{{ \App\Helpers\LocalizationHelper::t('auth.full_name') }}</label>
                ...
                <div>
                    <label for="password_confirmation" class="block text-sm font-medium text-gray-700 mb-1">{{ \App\Helpers\LocalizationHelper::t('auth.password_confirmation') }}</label>
                    <input id="password_confirmation" name="password_confirmation" type="password" required 
                           class="block w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500" 
                           placeholder="{{ \App\Helpers\LocalizationHelper::t('auth.password_confirmation_placeholder') }}">
                </div>
                <div>
                    <label for="role" class="block text-sm font-medium text-gray-700 mb-1">{{ \App\Helpers\LocalizationHelper::t('auth.role') }}</label>
                    <select id="role" name="role" required 
                            class="block w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500">
                        <option value="">{{ \App\Helpers\LocalizationHelper::t('auth.select_role') }}</option>
                        <option value="warehouse_employee" {{ old('role') == 'warehouse_employee' ? 'selected' : '' }}>
