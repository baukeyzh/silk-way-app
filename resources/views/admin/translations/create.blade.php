...
@section('title', translate('admin.translation_create_title') . ' - Silk Way')
...
<div class="space-y-6" x-data="translationCreateForm({{ $existingGroupsJson }})">
    ...
    <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
        <div>
            ...
            <h1 class="text-2xl font-bold text-slate-900">{{ translate('admin.translation_create_title') }}</h1>
            <p class="mt-1 text-sm text-slate-500">{{ translate('admin.translation_create_desc') }}</p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
        <form action="{{ route('admin.translations.store') }}" method="POST" novalidate>
            @csrf
            ...
            <div class="px-6 pt-6 pb-5 space-y-5">
                ...
                {{-- Key --}}
                <div>
                    <label for="key" class="block text-sm font-medium text-slate-700 mb-1.5">
                    ...
                    <input type="text"
                           ...
                           name="key"
                           value="{{ old('key') }}"
                           placeholder="{{ translate('admin.translation_key_placeholder') }}"
                           autocomplete="off"
                           spellcheck="false"
                           class="w-full rounded-lg border-slate-300 shadow-sm text-sm font-mono focus:border-indigo-500 focus:ring-indigo-500 @error('key') border-rose-400 bg-rose-50 @enderror">
                    @error('key')
                    ...
                    @else
                        <p class="mt-1.5 text-xs text-slate-400">{{ translate('admin.translation_key_hint') }}</p>
                    @enderror
                </div>

                {{-- Group (Alpine autocomplete) --}}
                <div class="relative" x-data="{ open: false, query: '{{ old('group', '') }}' }">
                    ...
                    <div x-show="open && filteredGroups().length > 0"
                        ...
                        <ul class="py-1 max-h-48 overflow-y-auto">
                            <template x-for="grp in filteredGroups()" :key="grp">
                                <li>
                                    <button type="button"
                                            @click="query = grp; open = false; $nextTick(() => document.getElementById('group').dispatchEvent(new Event('input')))"
                ...
                <div>
                    ...
                    <textarea id="description"
                              ...
                              class="w-full rounded-lg border-slate-300 shadow-sm text-sm focus:border-indigo-500 focus:ring-indigo-500 @error('description') border-rose-400 bg-rose-50 @enderror">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="mt-1.5 text-xs text-rose-600 flex items-center gap-1">
                            <i class="fas fa-circle-exclamation text-xs"></i>
                            {{ $message }}
            ...
            <div class="px-6 pt-5 pb-6 space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div class="rounded-xl border border-slate-200 overflow-hidden flex flex-col"
                         x-data="{ count: {{ mb_strlen(old($locale, ''), 'UTF-8') }} }">
                        ...
                        <div class="flex-1 flex flex-col px-4 py-3">
                            <textarea id="{{ $locale }}"
                                      ...
                                      class="flex-1 w-full resize-none rounded-lg border-slate-300 shadow-sm text-sm focus:border-indigo-500 focus:ring-indigo-500 @error($locale) border-rose-400 bg-rose-50 @enderror">{{ old($locale) }}</textarea>
                            @error($locale)
                                <p class="mt-1.5 text-xs text-rose-600 flex items-center gap-1">
                                    <i class="fas fa-circle-exclamation text-xs"></i>
                                    {{ $message }}
...
<script>
function translationCreateForm(existingGroups) {
    return {
        existingGroups: existingGroups || [],
        filteredGroups() {
            const q = (this.query || '').toLowerCase().trim();
            if (!q) return this.existingGroups.slice(0, 12);
            return this.existingGroups.filter(g => g.toLowerCase().includes(q)).slice(0, 12);
        },
        query: '{{ old('group', '') }}',
        open: false,
    };
